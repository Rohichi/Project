...
public class Solver {
	...
	static public TreeSet<Double> solve(double a, double b) {
		TreeSet<Double> ret = new TreeSet<Double>();
		ret.add(Double.valueOf((a == 0 ? -1. : -(b / a))));
		return ret;
	}
	
	static public TreeSet<Double> solve(double a, double b, double c) {
		if (a == 0.)
			return solve(b, c);
		TreeSet<Double> ret = new TreeSet<Double>(); 
	...
	static public TreeSet<Double> solve(double a, double b, double c, double d) {
		if (a == 0.)
			return solve(b, c ,d);
	...
	static public TreeSet<Double> solve(double a, double b, double c, double d, double e) {
		if (a == 0.)
		...
		double y0 = 0;
		TreeSet<Double> subret = solve(8, -4*p, -8*r,4*r*p-q*q);
		...
		for (Double double1 : ret) {
			subret.add(Double.valueOf(double1.doubleValue() -b/4));
		}
		return subret;
	}
}
